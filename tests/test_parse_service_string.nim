...
suite "actions":
  ...
  test "parse basic2":
    ...
    check srv_spec.request.fields[0].type.type == "bool"
    ...
    check srv_spec.response.fields[0].type.type == "int8"
  ...
  test "parse basic3":
    ...
    check srv_spec.request.fields[0].type.type == "bool"
    check srv_spec.request.fields[0].name == "foo"
    check srv_spec.request.fields[0].default_value
    check len(srv_spec.request.constants) == 0
    check len(srv_spec.response.fields) == 1
    check srv_spec.response.fields[0].type.type == "int8"
    check srv_spec.response.fields[0].name == "bar"
    check srv_spec.response.fields[0].default_value == 2
    check len(srv_spec.response.constants) == 0
  ...
  test "parse basic3":
    ...
    check srv_spec.request.constants[0].type == "bool"
    ...
    check srv_spec.response.constants[0].type == "int8"
    check srv_spec.response.constants[0].name == "BAR"
    check srv_spec.response.constants[0].value == 2
